...
import { SECRET, creaHash, validaPassword } from '../utils/utils.js';
...
export default class authController {
    ...
    static register = async (req, res) => {
        ...
        let cart = await carritoService.agregarCarrito({"carrito":{"products":[]}})
    ...
    static recupero01 = async ( req, res ) => {
       ...
       delete usuario.password 
       let token = jwt.sign(usuario, SECRET, {expiresIn:'30m'})
       ...
       let mensaje = `Usted ha solictado un cambio de contraseña. Si no fue usted por favor comunicarse con el admin. Para proceder haga click <a href=${url}> aqui </a>`
        ...
        try {
            ...
            res.redirect("/recupero01?mensaje=Recibirá un email en breve para su cambio de contraseña.")
        } catch (error) {
            res.setHeader('Content-Type','application/json')
            return res.status(500).json(
                {
    ...
    static getRecupero01 = (req, res) => {
        ...
        res.setHeader('Content-Type','text/html').status(200).render('recupero01',{mensaje})
    }
    ...
    static recupero02 = ( req, res ) => {
        ...
        let tokenValido = jwt.verify(token, SECRET, (err, decoded) => {
            if (err) {
        ...
        if (tokenValido){
            res.redirect(`/api/sessions/recupero02.html?token=${token}`)
        } 
    }

    static recupero03 = (req,res) => {

        let {password} = req.body
        let {token} = req.params

        let usuario = jwt.verify(token, SECRET, (err, decoded) => {
            if (err) {
              return res.status(401).json({ message: 'Token inválido o expirado', error: err.message });
            }
            res.status(200).json({ message: 'Token válido', decoded });
